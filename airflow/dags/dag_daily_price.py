...
with DAG(
    ...
    # Task 1: Ingest (Dùng command tĩnh)
    ingest_task = PythonOperator(
        task_id='ingest_daily_prices',
        python_callable=run_command_in_container,
        op_kwargs={
            'container_name': 'daily-price-pipeline',
            'command': 'python /app/ingestion/fetch_daily_price.py'
        }
    )

    # Task 2: Spark (Dùng Jinja qua templates_dict)
    process_task = PythonOperator(
        ...
        templates_dict={
            'rendered_command': (
                ...
                'hieunt.stock.spark.job.DailyPriceJob '
